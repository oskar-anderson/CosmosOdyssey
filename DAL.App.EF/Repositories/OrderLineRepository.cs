...
{
    private OrderLineMapper Mapper = new();
    private DbContext RepoDbContext;
    private DbSet<Domain.App.OrderLine> RepoDbSet;

    public OrderLineRepository(AppDbContext dbContext)
    {
        RepoDbContext = dbContext;
        RepoDbSet = RepoDbContext.Set<Domain.App.OrderLine>();
        if (RepoDbSet == null)
        {
    ...
    public async Task<OrderLine?> FirstOrDefault(Guid id)
    {
        return Mapper.DomainToDal(await RepoDbSet.FirstAsync(x => x.Id.Equals(id)));
    }

    public async Task<OrderLine> Add(OrderLine entity)
    {
    ...
    public async Task<OrderLine> RemoveAsync(Guid id)
    {
